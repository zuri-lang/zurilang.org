...
At the end of the chapter, you should be more motivated to learn Zuri.
...
- [Hello, World](#hello-world)
  ...
  - [Zuri the Calculator](#zuri-the-calculator)
...
Zuri comes with an interactive shell also knwon as the REPL (Read-Evaluate-Print-Loop) mode. This
...
To start Zuri in the REPL mode, simply type the following command in your terminal.
...
The Zuri REPL prompts for the next command with the greater-than (`%>`) sign or a continuation with
...
Following in the footsteps of legends, we are going to start learning Zuri by writing a simple _Hello, World_
application. Now, fire up your Zuri REPL, type the following and press ENTER.
...
Zuri program! The `'` surrounding our text is because we are in the REPL and serves as an easy way to identify
...
> Zuri script files can have any name and can end with any extension. But
> files that can be used as a module or form part of a package _MUST_ end with
> the `.b` extension.

Can you see it? Now you are a Zuri ninja! You now know two ways to print something to screen and you can use 
the REPL and a file to create a Zuri program. Give yourself a pat on the back and tell yourself weldone. You did good.
...
we take a look at using Zuri as a calculator?

When you type simple expressions into the Zuri REPL without any command or write an expression 
without an associated command within a condition, loop or at the end of a function, Zuri outputs 
...
In the examples above, we used Zuri as a simple arithemetic calculator. Notice that little `**`? That simply means
raise to power and 2 raised to the power of 5 equals 32.


> At this point, it's important to introduce you to three constants in Zuri. This constants cannot be changed and
> have reserved meanings and functions in Zuri. They include the boolean `true` and `false` as well as `nil` that 
> denotes the abscence of a value.


If all you ever want to do with Zuri is use it as a handy calculator, you are welcomed to stop here. 
